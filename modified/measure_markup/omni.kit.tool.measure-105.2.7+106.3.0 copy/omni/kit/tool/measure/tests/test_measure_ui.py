...
# TODO: Give the property UI a stable name. Unfortunatelly this needs code change in Kit
PROPERTY_UI_PATH = "Property//Frame/**/CollapsableFrame[0]/Frame[0]/ZStack[0]/VStack[0]/Frame[0]/VStack[0]/Frame[0]/VStack[0]"
...
class TestMeasureUI(TestMeasureBase):
    ...
    async def test_delete_from_manager(self):
        await create_one_measure()
        from ..manager import MeasurementManager
        self.assertEquals(len(MeasurementManager()._model.get_items()), 1)
        delete_btn = ui_test.find(MANAGER_UI_PATH + "/**/DeleteBtn")
        await delete_btn.click()
        await ui_test.wait_n_updates(3)
        self.assertEquals(len(MeasurementManager()._model.get_items()), 0)
    ...
    async def test_reset_measure_properties(self):
        ...
        property_ui = ui_test.find(PROPERTY_UI_PATH)
        self.assertNotEqual(property_ui, None)

        # NOTE: This test relies on the assumption that we start in the default statte
        elems = ((0, "control_state_measure:prop:axis_display", "WORLD"),
                 (1, "control_state_measure:prop:unit", "FEET"),
                 (2, "control_state_measure:prop:precision", "TENTH"),
                 (3, "control_state_measure:prop:label_size", "LARGE"))
        for idx, prop_name, change_to in elems:
            control = property_ui.find("HStack[{}]".format(idx))
            self.assertNotEqual(control, None)
            combobox = control.find("ZStack[0]/ComboBox[0]")
            reset_button = control.find("**/{}".format(prop_name))
    ...
    async def test_reset_measure_color(self):
        ...
        color_widget = ui_test.find(PROPERTY_UI_PATH + "/**/HStack[1]/ColorWidget[0]")
        ...
        rst_button = ui_test.find(PROPERTY_UI_PATH + "/**/control_state_measure:prop:label_color")
