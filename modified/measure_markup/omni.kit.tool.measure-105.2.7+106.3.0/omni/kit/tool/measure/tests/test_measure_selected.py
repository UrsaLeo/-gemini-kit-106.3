...
class TestMeasureSelected(TestMeasureBase):
    ...
    async def test_measure_selected_payloads_min_max(self):
        ...
        def create_payload(payload_prim_path: str):
            ...
            payload_prim.GetPayloads().AddPayload(
                Sdf.Payload(new_stage.GetRootLayer().identifier, "/Root")
            )
        ...
        self.assertTrue(
            Gf.IsClose(omni.usd.get_world_transform_matrix(payload_prim_a).ExtractTranslation(), payload_a_translation, 1e-6)
        )
    ...
    async def test_measure_save_export_and_open(self):
        ...
        """
        import tempfile
        from omni.kit.tool.measure.system.export import _export_csv, ExportPanel
