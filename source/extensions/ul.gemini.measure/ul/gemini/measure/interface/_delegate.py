...
class MeasurePanelDelegate(ui.AbstractItemDelegate):
    ...
    def _on_label_click(self, button, field, label, uuid: int):
        ...
        self.subscription = field.model.subscribe_end_edit_fn(
            lambda m, f=field, l=label: _on_end_edit(m, f, l)
        )
        ...
        if level == 1 and issubclass(item.__class__, MeasurePrim):
            ...
            if column_id == 0:  # Visibility
                with ui.ZStack(width=20, height=20):
                    ...
                    button = ui.ImageWithProvider(image_provider, width=20, height=20, style=STYLE_BUTTON_VISIBILITY, identifier="VisibilityBtn")
                    button.set_mouse_pressed_fn(lambda x, y, btn, m, b=button, i=image_provider, uuid=item.uuid: self._on_visibility_clicked(b, i, uuid))
                self.__ui_cache[item.payload.uuid].visibility_btn = button

            elif column_id == 1:  # Go to
                with ui.ZStack(width=20, height=20):
                    ui.Rectangle(style=STYLE_BUTTON_GO_TO)
                    button = ui.ImageWithProvider(ui.VectorImageProvider(source_url=get_icon_path("go_to")), width=20, height=20, style=STYLE_BUTTON_GO_TO, identifier="GotoBtn")
                    button.set_mouse_pressed_fn(lambda x, y, btn, m, b=button, uuid=item.uuid: self._on_go_to_clicked(b, uuid))
                ...
                label_stack.set_mouse_double_clicked_fn(lambda x,y,b,m, f=field, l=label, u=item.payload.uuid: self._on_label_click(b, f, l, u))
                ...
                if item.payload.tool_mode in [MeasureMode.ANGLE, MeasureMode.AREA]:
                    unit_type = "°" if item.payload.tool_mode == MeasureMode.ANGLE else f"{item.payload.unit_type}²"
                else:
                    unit_type = item.payload.unit_type
                ...
                with ui.HStack():
                    ...
                    with ui.ZStack(width=20, height=20):
                        ...
                        button = ui.ImageWithProvider(ui.VectorImageProvider(source_url=get_icon_path("tool_delete")), width=20, height=20, style=STYLE_BUTTON_DELETE, identifier="DeleteBtn")
