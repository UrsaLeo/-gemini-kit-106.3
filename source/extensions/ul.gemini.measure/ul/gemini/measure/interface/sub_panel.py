...
class MarkupPanel(SubPanelBase):
    ...
    def _draw(self) -> Union[ui.Stack, ui.CollapsableFrame]:
        print("Drawing MarkupListWindow...")
        _frame = ui.CollapsableFrame(self._name, name="frame", style=STYLE_PLACEMENT_PANEL, collapsed=True)

        with _frame:
            with ui.VStack(name="container", height=0, spacing=8):
                widget = MarkupListWindow()
                print("MarkupListWindow created:", widget)
...
class GlobalPanel(SubPanelBase):
    def __init__(self):
        ...
        self._selection_changed_sub: int = StateMachine().subscribe_to_stage_event(
            ...
            ou.StageEventType.SELECTION_CHANGED
        )
        ...
        with _stack:
            ...
            with ui.HStack(spacing=0):
                ...
                with ui.HStack(spacing=4):
                    ui.Spacer()
                    ui.Spacer(width=24)
                    for tool in MeasureMode:
                        if tool.value in [0, 1, 2, 3, 4]:  # TODO: when all are ready we need to use tool.value >= 1
                            ToolButton(tool, StateMachine().set_creation_state)

                        if tool.value == 6:  # Requires a different callback for MeasureSelected
                            ui.Rectangle(width=1, height=24)  # Vertical Separator
                            self._measure_selected = ToolButton(tool, self._on_measure_selected, enabled=False)
...
class PlacementPanel(SubPanelBase):
    def __init__(self):
        self._snap_state = {
            SnapTo.CUSTOM: [SnapMode.VERTEX],
            SnapTo.PERPENDICULAR: [SnapMode.SURFACE]
        }
...
class DisplayPanel(SubPanelBase):
    ...
    def _draw(self) -> ui.CollapsableFrame:
        _frame = ui.CollapsableFrame(
            self._name, name="frame", style=STYLE_DISPLAY_PANEL, identifier="DisplayPanal", collapsed=True
        )
...
class ManagePanel(SubPanelBase):
    def __init__(self):
        ...
        self._selection_sub = StateMachine().subscribe_to_stage_event(
            self.__on_stage_selection_changed,
            ou.StageEventType.SELECTION_CHANGED
        )
    ...
    def _draw_menu(self):
        with ui.Frame():
            ...
            with self._filter_menu:
                ...
                ui.MenuItem(
                   "Reset",
                   triggered_fn=self._on_filter_reset,
                ...
                filter_angle = ui.MenuItem(
                    "Angle",
                    checkable=True,
                    checked_changed_fn=lambda c: self._on_filter_by(MeasureMode.ANGLE, c)
                )
                filter_area = ui.MenuItem(
                    "Area",
                    checkable=True,
                    checked_changed_fn=lambda c: self._on_filter_by(MeasureMode.AREA, c)
                )
                filter_selected = ui.MenuItem(
                    "Selected",
                    checkable=True,
                    checked_changed_fn=lambda c: self._on_filter_by(MeasureMode.SELECTED, c)
                )
            ...
            with self._options_menu:
                ...
                ui.Separator()
                option_csv = ui.MenuItem(
                    "Export to CSV",
                    triggered_fn=self._on_export_csv
                )
                ui.Separator()
                option_hide_all = ui.MenuItem(
                    "Hide All",
                    triggered_fn=self._on_hide_all
                )
                option_unhide_all = ui.MenuItem(
                    "Unhide All",
                    triggered_fn=self._on_unhide_all
                )
                ui.Separator()
                option_delete_all = ui.MenuItem(
                    "Delete All",
                    triggered_fn=MeasurementManager().delete_all
                )
    ...
    def _draw(self) -> ui.CollapsableFrame:
        self._draw_menu()
        _frame = ui.CollapsableFrame(
            self._name, name="frame", identifier="ManagePanel", style=STYLE_DISPLAY_PANEL, collapsed=True
        )
            ...
            with ui.VStack(name="container", spacing=4, height=0):
                ...
                with ui.HStack(spacing=4):
                    with ui.ZStack(style=STYLE_SEARCH_FIELD):
                        self._search_field = ui.StringField(model=self.__query_model, name="search", identifier="Search")
                ...
                with scroll_frame:
                    self._measurement_view = ui.TreeView(
                        ...
                        root_visible=False,
                        header_visible=True,
                        column_widths=[ui.Pixel(45), ui.Pixel(45), ui.Fraction(1), ui.Fraction(1), ui.Pixel(45), ui.Pixel(45)],
                        style={"margin": 0.5},
