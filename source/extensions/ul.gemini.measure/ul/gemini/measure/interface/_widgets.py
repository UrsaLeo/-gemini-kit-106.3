...
from ..common import SnapMode, MeasureCreationState, MeasureMode, MeasureState, UserSettings
...
class ResetButton(ui.Widget):
    ...
    def __build(
        ...
        def _reset_check_fn(value: Any, button: int):
            if button == 0 and StateMachine().tool_creation_state in [MeasureCreationState.NONE, MeasureCreationState.START_SELECTION]:  # left click
                reset_fn(reset_val)
...
class ToolButton(ui.Button):
    def __init__(
        self,
        tool: MeasureMode,
        clicked_fn: Callable[[MeasureMode], None],
        enabled: bool = True
    ):
...
class GroupBoxBase(metaclass=ABCMeta):
    def __init__(self, group_name: str, draw_label=True, content_name: str = "group_content", bounds_name: str = "group_bounds"):
...
class SnapGroupBox(GroupBoxBase):
    ...
    def __init__(self, group_name: str):
        ...
        super().__init__(group_name, draw_label=False, content_name="snap_group_content", bounds_name="snap_group_bounds")
        ...
        with ui.HStack(spacing=24, style=STYLE_CHECKBOX):
            ...
            with ui.VStack(spacing=button_size, width=0):
                with ui.HStack(spacing=0):
                    self._surface = ui.RadioButton(radio_collection=self._snap_collection, style=STYLE_RADIO_BUTTON, width=button_size)
                ...
                with ui.HStack(spacing=0):
                    self._vertex = ui.RadioButton(radio_collection=self._snap_collection, style=STYLE_RADIO_BUTTON, width=button_size, enabled=enable_geometry_snap)
                    ui.Spacer(width=label_spacing)
                    self._vertex_label = ui.Label("Vertex", width=label_width, tooltip=TOOLTIP_VERTEX, enabled=enable_geometry_snap)
                    self._display_order.append(SnapMode.VERTEX)
                with ui.HStack(spacing=0):
                    self._mid = ui.RadioButton(radio_collection=self._snap_collection, style=STYLE_RADIO_BUTTON, width=button_size, enabled=enable_geometry_snap)
                    ui.Spacer(width=label_spacing)
                    self._mid_label = ui.Label("Mid Point", width=label_width, tooltip=TOOLTIP_MIDPOINT, enabled=enable_geometry_snap)
                    self._display_order.append(SnapMode.MIDPOINT)
            with ui.VStack(spacing=button_size, width=0):
                with ui.HStack(spacing=0):
                    self._center = ui.RadioButton(radio_collection=self._snap_collection, style=STYLE_RADIO_BUTTON, width=button_size)
                    ui.Spacer(width=label_spacing)
                    self._center_label = ui.Label("Center", width=label_width, tooltip=TOOLTIP_CENTER)
                    self._display_order.append(SnapMode.CENTER)
                with ui.HStack(spacing=0):
                    self._edge = ui.RadioButton(radio_collection=self._snap_collection, style=STYLE_RADIO_BUTTON, width=button_size, enabled=enable_geometry_snap)
                    ui.Spacer(width=label_spacing)
                    self._edge_label = ui.Label("Edge", width=label_width, tooltip=TOOLTIP_EDGE, enabled=enable_geometry_snap)
                    self._display_order.append(SnapMode.EDGE)
                with ui.HStack(spacing=0):
                    self._pivot = ui.RadioButton(radio_collection=self._snap_collection, style=STYLE_RADIO_BUTTON, width=button_size)
                    ui.Spacer(width=label_spacing)
            ...
            if not enable_geometry_snap:
                if UserSettings().session.snap_vertex is True or UserSettings().session.snap_midpoint is True or UserSettings().session.snap_edge is True:
                    UserSettings().session.snap_vertex = UserSettings().session.snap_midpoint = UserSettings().session.snap_edge = False
    ...
    def lock_snaps(self, locked: bool, exclude: List[SnapMode] = []) -> None:
        ...
        self._vertex_label.enabled = self._vertex.enabled = (not locked or SnapMode.VERTEX in exclude) and enable_geometry_snap
        self._mid_label.enabled = self._mid.enabled = (not locked or SnapMode.MIDPOINT in exclude) and enable_geometry_snap
        self._edge_label.enabled = self._edge.enabled = (not locked or SnapMode.EDGE in exclude) and enable_geometry_snap
    ...
    def _geometry_snap_enabled(self):
        ...
        if not enabled:
            carb.log_warn("Vertex, Edge and Mid Point snap requires `/rtx-transient/scenedb/useUniformsReindexing` setting to be true")
        return enabled
