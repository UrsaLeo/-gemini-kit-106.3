...
from ..common import MeasureMode, DisplayAxisSpace, convert_distance_and_units, convert_area_to_units
...
class MultiPointCompute(MeasureCompute):
    ...
    def execute(self, payload: MeasurePayload) -> Optional[ComputeOutput]:
        ...
        computed_points = [model.computed_point for model in self._point_models]
        sub_point_lengths = [(computed_points[i] - computed_points[i+1]).GetLength() for i in range(len(computed_points) - 1)]
...
class SelectedCompute(MeasureCompute):
    ...
    @wp.kernel
    def __find_closest_points(points_a: wp.array(dtype=wp.vec3d), points_b: wp.array(dtype=wp.vec3d), out_distances: wp.array(dtype=wp.float64), out_indices: wp.array(dtype=wp.int32)):
        tid = wp.tid()

        point = points_a[tid]
        closest_index = wp.int32(0.0)
        closest_distance = wp.float64(1.7976931348623157e+308)
    ...
    @wp.kernel
    def __find_furthest_points(points_a: wp.array(dtype=wp.vec3d), points_b: wp.array(dtype=wp.vec3d), out_distances: wp.array(dtype=wp.float64), out_indices: wp.array(dtype=wp.int32)):
        tid = wp.tid()
    ...
    def __compute_nearest(self, points_a: List[Gf.Vec3d], points_b: List[Gf.Vec3d], payload: MeasurePayload) -> ComputeOutput:
        ...
        wp.launch(SelectedCompute.__find_closest_points, num_points_a, [wp.array(points_a, dtype=wp.vec3d), wp.array(points_b, dtype=wp.vec3d), out_distances, out_indices])
    ...
    def __compute_furthest(self, points_a: List[Gf.Vec3d], points_b: List[Gf.Vec3d], payload: MeasurePayload) -> ComputeOutput:
        num_points_a = len(points_a)
        out_distances = wp.empty(num_points_a, dtype=wp.float64)
        out_indices = wp.empty(num_points_a, dtype=wp.int32)

        wp.launch(SelectedCompute.__find_furthest_points, num_points_a, [wp.array(points_a, dtype=wp.vec3d), wp.array(points_b, dtype=wp.vec3d), out_distances, out_indices])
    ...
    def execute(self, payload: MeasurePayload) -> Optional[ComputeOutput]:
        ...
        return self.__compute_nearest(points_a, points_b, payload) if is_min else self.__compute_furthest(points_a, points_b, payload)
