...
class DiameterModel(ViewportModeModel):
    ...
    def _on_moved(self, coords: Sequence[float], result: omni.kit.raycast.query.RayQueryResult):
        if self.creation_state not in [MeasureCreationState.NONE, MeasureCreationState.FINALIZE]:
            ...
            self._snap_data: Optional[Dict[str, Any]] = MeasureSnapProviderManager().get_snap_position(
                coords, result
            )
    ...
    def _on_save(self):
        ...
        payload.points = MeasurePayload.world_to_local_points([self._start_point.vector, self._mid_point.vector, self._end_point.vector], payload.prim_paths)
