...
from ._scene_widget import MeasureSceneLabel, MeasureAxisStackLabel
...
class PointToPointModel(ViewportModeModel):
    ...
    def __init__(self, viewport_api):
        ...
        with self._label_root:
            ...
            self._ui_perpendicular: sc.Line = sc.Line([0,0,0], [0,0,0], color=[1,1,0,.5], thickness=2, visible=False)
    ...
    def _draw_xyz(self) -> None:
        ...
        if y_dist != 0:
            y_line = sc.Line([*y_start], [*y_end], color=ui.color("#71A376"), thickness=3)  # x_line X END to end point Y
            # y_txt, y_unit = self._value_to_unit(y_dist)
    ...
    def _on_save(self):
        ...
        payload.points = MeasurePayload.world_to_local_points([self._start_point.vector, self._end_point.vector], payload.prim_paths)
