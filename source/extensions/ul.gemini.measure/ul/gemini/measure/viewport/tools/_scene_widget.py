...
class SnapMarker:
    ...
    def _widget_build_fn(self) -> None:
        with self.__root:
            ...
            with sc.Transform(transform=sc.Matrix44.get_translation_matrix(SNAP_SIZE/2, -SNAP_SIZE/2-3, 0), scale_to=sc.Space.SCREEN):
...
class MeasureSceneLabel:
    def __init__(
        ...
        self._click_gesture = sc.ClickGesture(
            ...
            manager=GesturePreventionManager()
        )
        ...
        with self._root:
            with sc.Transform(scale_to=sc.Space.SCREEN):
                ...
                with self._label_xform:
                    self._label = sc.Label("", size=text_size, color=[0,0,0,1], alignment=ui.Alignment.LEFT_CENTER)  # Aligns right
                self._rectangle = sc.Rectangle(width=135, height=45, color=[1,1,1,1], wireframe=False)  # This will have a gesture
                self._icon_xform = sc.Transform(transform=sc.Matrix44.get_translation_matrix(-45, 0, 0)) # Offset the colored box
                with self._icon_xform:
                    ...
                    if self._axis == MeasureAxis.NONE:
                        ...
                        self._icon = sc.Label(self._axis.name, size=24, color=[1,1,1,1], alignment=ui.Alignment.CENTER)
    ...
    def __update_label_visuals(self):
        ...
        rect_width = int((self._icon_bg.width * 1.5 * size_bias) + (10 * char_len))  # (1.5*icon_width) + 10 units per character
    ...
    def __draw_icon(self, axis: MeasureAxis, add_delete:bool = False) -> sc.Transform:
        ...
        with xform:
            ...
            if axis == MeasureAxis.NONE:
                ...
                icon_path = get_icon_path("scene_icon_tool_delete") if self.selected else get_icon_path(f"scene_icon_tool_{self._mode.name.lower()}")
            ...
            with sc.Transform(scale_to=sc.Space.SCREEN):
                ...
                with sc.Transform(transform=m_pos):  # vertical
                    ...
                    with self._m_label_xform:
                        self._m_label = sc.Label("", size=text_size, color=[0,0,0,1], alignment=ui.Alignment.LEFT_CENTER)
                ...
                with sc.Transform(transform=x_pos):  # vertical
                    ...
                    with self._x_label_xform:
                        self._x_label = sc.Label("", size=text_size, color=[0,0,0,1], alignment=ui.Alignment.LEFT_CENTER)
                ...
                with sc.Transform(transform=y_pos):  # vertical
                    ...
                    with self._y_label_xform:
                        self._y_label = sc.Label("", size=text_size, color=[0,0,0,1], alignment=ui.Alignment.LEFT_CENTER)
                ...
                with sc.Transform(transform=z_pos):  # vertical
                    ...
                    with self._z_label_xform:
                        self._z_label = sc.Label("", size=text_size, color=[0,0,0,1], alignment=ui.Alignment.LEFT_CENTER)
