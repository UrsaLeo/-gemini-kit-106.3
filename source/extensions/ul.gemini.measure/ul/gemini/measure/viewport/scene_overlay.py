...
from carb.settings import get_settings, ISettings
...
class MeasureSceneOverlay():
    ...
    def __init__(self, on_enter_pressed_fn: Optional[Callable] = None):
        ...
        self._nav_visibility_sub = self._settings.subscribe_to_node_change_events(
            self.NAVBAR_VISIBILITY_PATH,
            lambda item, name="navigation": self.__on_visibility_changed(item, name)
        )

        self._timeline_visibility_sub = self._settings.subscribe_to_node_change_events(
            self.TIMELINE_MINIBAR_PATH,
            lambda item, name="timeline_minibar": self.__on_visibility_changed(item, name)
        )
    ...
    def __on_creation_state_changed(self, state: MeasureCreationState) -> None:
        if self._button_stack:
            self._visibility = state == MeasureCreationState.END_SELECTION and StateMachine().tool_mode in self.FINALIZE_MODES
            self._button_stack.visible = self._visibility
    ...
    def _on_key_pressed(self, key: KeyboardInput) -> None:
        ...
        if (tool := StateMachine().tool_mode) in self.FINALIZE_MODES and StateMachine().tool_creation_state == MeasureCreationState.END_SELECTION:
