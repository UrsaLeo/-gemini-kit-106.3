...
from typing import cast, Callable, Dict, Type, Union
...
from carb.input import (
    ...
    KeyboardEvent, KeyboardEventType, KeyboardInput,
...
class StateMachineStageEventManager:
    def __init__(self):
        self.__events: Dict[int, StateMachineEvent] = {
            int(StageEventType.OPENING): StateMachineEvent(Callable),
            int(StageEventType.OPENED): StateMachineEvent(Callable),
            int(StageEventType.CLOSING): StateMachineEvent(Callable),
            int(StageEventType.CLOSED): StateMachineEvent(Callable),
            int(StageEventType.SAVED): StateMachineEvent(Callable),
            int(StageEventType.SELECTION_CHANGED): StateMachineEvent(Callable)
        }
    ...
    def exec_event(self, event: IEvent) -> None:
        stage_event = self.__events.get(event.type, lambda *args, **kwargs: None)
        stage_event()
...
class StateMachineLayerEventManager:
    def __init__(self):
        self.__events: Dict[int, StateMachineEvent] = {
            ...
            int(LayerEventType.LIVE_SESSION_STATE_CHANGED): StateMachineEvent(Callable[[LayerEventPayload, bool], None]),
...
class StateMachine:
    ...
    def __singleton_init__(self):
        ...
        self.__stage_event_sub = get_context().get_stage_event_stream().create_subscription_to_pop(
            self.__on_stage_event,
            name="omni.kit.tool.measure::StateMachine:StageEvent"
        )

        self.__layers = cast(Layers, get_layers(get_context()))
        self.__layer_event_sub = self.__layers.get_event_stream().create_subscription_to_pop(
            self.__on_layer_event,
            name="omni.kit.tool.measure::StateMachine:LayerEvent"
        )
        ...
        self.__stage_listener = Tf.Notice.Register(
            ...
            self.__on_stage_objects_changed,
    ...
    # Stage Event Logic
    def __on_stage_event(self, event: IEvent) -> None:
    ...
    # Layer Event Logic
    def __on_layer_event(self, event: IEvent) -> None:
    ...
    def subscribe_to_layer_event(self, callback: Callable[["LayerEventPayload", bool], None], event_type: LayerEventType) -> int:
